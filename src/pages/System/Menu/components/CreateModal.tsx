...
export type Props = {
  open: boolean;
  onCancel: () => void;
  onSubmit: (values: API.MenuAddRequest) => void;
};
...
const CreateModal: React.FC<Props> = (props) => {
  ...
  const formRef = useRef<ProFormInstance<API.MenuAddRequest>>();

  return (
    <ModalForm<API.MenuAddRequest>
      title="新建菜单"
      width={500}
      open={open}
      autoFocusFirstInput
      ...
      onFinish={async (values) => {
        // console.log('menu create finish',values);
        onSubmit(values);
        return true;
      }}
      formRef={formRef}
